//------------------------------------------------------------------------------
// <auto-generated>
//    This code was generated from a template.
//
//    Manual changes to this file may cause unexpected behavior in your application.
//    Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace DMS.Repository
{
    using System;
    using System.Data.Entity;
    using System.Data.Entity.Infrastructure;
    
    public partial class DMSEntities : DbContext
    {
        public DMSEntities()
            : base("name=DMSEntities")
        {
        }
    
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            throw new UnintentionalCodeFirstException();
        }
    
        public DbSet<tblAppoiment> tblAppoiments { get; set; }
        public DbSet<tblDoctor> tblDoctors { get; set; }
        public DbSet<tblDoctorType> tblDoctorTypes { get; set; }
        public DbSet<tblEmployee> tblEmployees { get; set; }
        public DbSet<tblInsurance> tblInsurances { get; set; }
        public DbSet<tblInvoice> tblInvoices { get; set; }
        public DbSet<tblLogin> tblLogins { get; set; }
        public DbSet<tblOfficeVisit> tblOfficeVisits { get; set; }
        public DbSet<tblPatient> tblPatients { get; set; }
        public DbSet<tblProcedure> tblProcedures { get; set; }
        public DbSet<tblUserType> tblUserTypes { get; set; }
    }
}
